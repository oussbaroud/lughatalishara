...
router.get('/manage', loggedIn, (request, response) => {
    if(request.user){
        response.render('homeforadmins', { title: 'الرئيسية', home: 'page-active', statusAdmin: 'true', css: ['styleforvisitors.css'], js: [''] });
    }else{
        response.redirect('/manage/login');
    }
})
router.get('/manage/dictionary', (request, response) => {
    response.render('managewords', { title: 'القاموس', manageWords: 'page-active', statusAdmin: 'true', css: ['styleforusers.css'], js: ['managewords.js', 'pagifortable.js'] })
})
router.get('/manage/letters', (request, response) => {
    response.render('manageletters', { title: 'الحروف', manageLetters: 'page-active', statusAdmin: 'true', css: ['styleforusers.css'], js: ['manageletters.js', 'pagifortable.js'] })
})
router.get('/manage/admins', (request, response) => {
    response.render('manageadmins', { title: 'المشرفين', manageAdmins: 'page-active', statusAdmin: 'true', css: ['styleforusers.css'], js: ['manageadmins.js', 'pagifortable.js'] })
})
...
router.get('/manage/login', loggedInForAdmins, (request, response) => {
    if(request.user){
        ...
        response.render('loginforadmins', { title: 'تسجيل الدخول', statusVisitor: 'true', css: ['styleforvisitors.css'], js: ['login.js'] })
    }
})
